...
const ledeOne = `Every requirement of the first release, in one place: ${total} items, each with what the platform does today, whether it is in the release, a ranked candidate, or not planned, and the test that will prove it works. It is built from your Core list of September 14, three dozen screens in your demo, and 109 decisions found by reading the code that runs today. ${waiting.length ? `${waiting.length === 1 ? 'One item is' : `${word(waiting.length)} items are`} waiting on you; the rest say what will be built unless you say otherwise.` : 'Nothing is waiting on you; every item says what will be built unless you say otherwise.'}`;
const found = `${total} items: your Core list of September 14, 2026, the features your demo shows, and 109 decisions found on September 4 by reading the code${extra - 106 > 0 ? `, plus ${word(Math.min(extra - 106, 9))} follow-up ${extra - 106 === 1 ? 'question' : 'questions'} raised since` : ''}.`;
